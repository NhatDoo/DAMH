...
const RankedResults = ({ rankedResults, setRankedResults, setShowAlternativeInput, saveToNeo4j }) => {
  useEffect(() => {
    if (rankedResults && rankedResults.alternatives && rankedResults.consistencyRatio !== undefined && rankedResults.consistencyRatio <= 0.1) {
      const canvas = document.getElementById('chartCanvas');
      if (canvas) {
        const ctx = canvas.getContext('2d');
        if (ctx) {
          ...
          const alternatives = rankedResults.alternatives;
          const maxScore = Math.max(...alternatives.map(a => a.score * 100)) || 1; // Tránh chia cho 0
          const barWidth = canvas.width / alternatives.length - 10;
          const chartHeight = canvas.height - 40;
          ...
          alternatives.forEach((item, index) => {
            const x = index * (barWidth + 10) + 5;
            const height = (item.score * 100 / maxScore) * chartHeight || 0;
            const y = chartHeight - height + 20;
  ...
  if (!rankedResults || !rankedResults.alternatives || rankedResults.consistencyRatio === undefined) {
