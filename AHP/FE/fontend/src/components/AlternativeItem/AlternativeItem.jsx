...
//   const [newAlternative, setNewAlternative] = useState('');
//   const [alternativesList, setAlternativesList] = useState([]);
//   const [criteriaMatrix, setCriteriaMatrix] = useState([]); // Thêm state cho ma trận tiêu chí
//   const [rankedResults, setRankedResults] = useState(null);
//   const [showAlternativeInput, setShowAlternativeInput] = useState(false);
...
//   // Khởi tạo ma trận tiêu chí khi danh sách tiêu chí thay đổi
//   useEffect(() => {
...
//               setRankedResults={setRankedResults}
//               setShowAlternativeInput={setShowAlternativeInput}
//               criteriaList={criteriaLabels}
//               setCriteriaList={setCriteriaLabels}
...
//         final_score: score || 0,
//         criterion_scores: criterionScores,
//         criteria_list: criteriaLabels,
//         criteria_comparison_matrix: editableMatrix.flat(),
//         consistency_ratio: currentCR,
//       };
//       console.log('Sending data to addTchiUser:', JSON.stringify(data, null, 2));
//       const result = await addTchiUser(data);
//       alert(`Phương án gợi ý "${alternative}" đã được thêm vào backend.`);
//       onAddClick(result); // Pass the result to trigger a refresh
//     } catch (error) {
//       console.error('Failed to add suggested alternative:', error);
//       alert(`Lỗi khi thêm phương án gợi ý: ${error.message}`);
//     }
...
//       await deleteTchiUser(id);
//       alert(`Phương án "${alternative}" đã được xóa.`);
//       onDeleteClick(id); // Trigger parent refresh
//     } catch (error) {
//       console.error('Failed to delete alternative:', error);
//       alert(`Lỗi khi xóa phương án: ${error.message}`);
//     }
...
//                     onClick={(e) => {
//                       e.stopPropagation();
//                       onScoreSave(alternative, isTchiUser, itemId, criterion, editingValue);
//                     }}
//                   >
//                     Lưu
//                   </button>
...
import useMatrix from './hooks/useMatrix';
import useNeo4j from './hooks/useNeo4j.jsx';
import { addTchiUser, deleteTchiUser, updateTchiUser } from '../../services/apiService.js';
import Chatbot from './Chatbot';
...
const AlternativeItem = ({
  ...
  const handleAddSuggestedAlternative = async (e, alternative, score, criterionScores) => {
    ...
    try {
      ...
      const flatMatrix = editableMatrix.flat().map(val => parseFloat(val.toFixed(2)));
  ...
  const handleScoreSave = async (alternative, isTchiUser, itemId, criterion, value) => {
    ...
    try {
      ...
      console.log('Sending data to updateTchiUser:', JSON.stringify(data, null, 2));
      const result = await updateTchiUser(itemId, data);
      alert(`Điểm tiêu chí cho "${alternative}" đã được cập nhật.`);
      onScoreSave(result); // Notify parent to refresh
    } catch (error) {
              ...
              {editingCriterion === criterion && (
                <>
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      handleScoreSave(alternative, isTchiUser, itemId, criterion, editingValue);
                    }}
                  >
                    Lưu
                  </button>
...
//                   />
//                   <p>Tỷ lệ nhất quán (CR): {currentCR.toFixed(4)}</p>
//                   <p>Lambda Max: {(lambdaMax || 0).toFixed(4)}</p>
//                   <p>Vector ưu tiên: {priorityVector ? priorityVector.map(val => val.toFixed(4)).join(', ') : '[]'}</p>
//                 </div>
//               ) : (
//                 <div className="matrix-display">
//                   <h4>Ma trận so sánh</h4>
